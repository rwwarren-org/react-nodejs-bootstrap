...
var react = __dirname + '/../src/inc/react.min.js';
var reactDom = __dirname + '/../src/inc/react-dom.min.js';
var reactFile = 'react.min.js';
var reactDomFile = 'react-dom.min.js';
...
var JSGulp = {
  ...
  getBundleStream: function() {
    ...
    gulp.src([react, reactDom]).pipe(gulp.dest(outputPath));
    if(argv.production) {
      // minification
  ...
  clean: function() {
    ...
    del([outputPath + '/' + reactDomFile], function(err, paths) {
      if(err) {
        console.error('Error cleaning JS: ' + err.toString());
        return;
      }
      console.log('Cleaned React Dom JS');
    });
  }
};
